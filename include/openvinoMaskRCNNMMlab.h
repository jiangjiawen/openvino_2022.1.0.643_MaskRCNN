...
class openvinoMaskRCNNMMlab{
public:
    explicit openvinoMaskRCNNMMlab(const std::string model_path);
    ~openvinoMaskRCNNMMlab();

    void initInputOutputInfo();
    void printInputAndOutputsInfo(const ov::Model& network);

    void preprocessImage(const cv::Mat& oimg);
    void detect(const cv::Mat &image, int64 *&pred_l, float *&pred_b, float *&pred_m, int& count);
